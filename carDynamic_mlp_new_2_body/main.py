...
from utils import (get_data, load_trajectory_segments,
                    ...
                    state_delta_body_frame, apply_body_frame_delta)
...
def kinematic_physics_loss(pred_delta_physical, cur_abs_physical, state_cols, dt=0.05):
    idx_vx = state_cols.index('car_linear_velocity_x') # lateral
    idx_vy = state_cols.index('car_linear_velocity_y') # forward
    idx_wz = state_cols.index('car_angular_velocity_z')
    idx_sin = state_cols.index('sin_theta')
    idx_cos = state_cols.index('cos_theta')
    
    idx_pos_x = state_cols.index('car_position_x') 
    idx_pos_y = state_cols.index('car_position_y') 

    # Extract current velocities
    vx = cur_abs_physical[..., idx_vx]
    vy = cur_abs_physical[..., idx_vy]
    wz = cur_abs_physical[..., idx_wz]

    # Calculate the model's predicted next velocities
    next_vx = vx + pred_delta_physical[..., idx_vx]
    next_vy = vy + pred_delta_physical[..., idx_vy]
    next_wz = wz + pred_delta_physical[..., idx_wz]

    # 1. Slip Loss (Lateral stability)
    slip_mask = (torch.abs(wz) < 1.0).float()
    loss_slip = torch.mean((next_vx ** 2) * slip_mask)

    # 2. Upgraded XY Kinematic Loss (Trapezoidal Rule)
    # Averages the current and next velocity for high-precision integration
    expected_delta_forward = ((vy + next_vy) / 2.0) * dt
    expected_delta_lateral = ((vx + next_vx) / 2.0) * dt
    
    loss_xy = torch.mean((pred_delta_physical[..., idx_pos_x] - expected_delta_forward) ** 2 + 
                         (pred_delta_physical[..., idx_pos_y] - expected_delta_lateral) ** 2)

    # 3. Upgraded Theta Loss (Trapezoidal Rule)
    # Forces the predicted dtheta to strictly match the average yaw rate
    expected_dtheta = ((wz + next_wz) / 2.0) * dt  
...
def _multistep_training_step(model, segments, scaler_in, scaler_out, torch_scaler_out, optimizer, device,
                              criterion, batch_size, k_steps, seq_length, grad_clip, rng,
                              weight_xy=20.0, weight_theta=20.0, weight_slip=50.0):
    ...
    for step in range(k_steps):
        ...
        target_delta = state_delta_body_frame(cur_abs, future_abs[:, step, :])
        ...
        # pred_delta's position channels are (forward, lateral) in the body
        # frame of cur_abs -- rotate them back to world frame before adding.
        pred_delta = scaler_out.inverse_transform(pred_delta_scaled.detach().cpu().numpy())
        next_abs = apply_body_frame_delta(cur_abs, pred_delta)
        for b in range(batch_size):
            next_abs[b] = renormalize_sincos(next_abs[b])
        new_delta = state_delta_body_frame(cur_abs, next_abs)
        ...
        for seg_idx in segment_indices:
            ...
            for start in starts[:starts_per_segment]:
                ...
                for h in range(roll_len):
                    ...
                    cur_abs = abs_state[-1]
                    pred_abs = renormalize_sincos(apply_body_frame_delta(cur_abs, pred_delta))
                    ...
                    new_delta = state_delta_body_frame(cur_abs, pred_abs)
...
def train_model(model, train_loader, val_loader, epochs, learning_rate, writer, device,
                 ...
                 multistep_k_start=1, multistep_k_warmup_epochs=1,
                 weight_xy=20.0, weight_theta=20.0, weight_slip=50.0):
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in train_loader:
            ...
            if noise_std > 0:
                ...
                # Add SLIGHT noise to control commands to simulate MPC jitter
                noise[..., :N_CONTROL] = torch.randn_like(inputs[..., :N_CONTROL]) * (noise_std * 0.5)
                inputs = inputs + noise
        ...
        if use_multistep:
            ...
            # dynamically calculate the horizon k based on the current epoch
            progress = min(1.0, epoch / max(1, multistep_k_warmup_epochs))
            current_k = int(multistep_k_start + (multistep_k - multistep_k_start) * progress)
            ...
            for _ in range(multistep_batches_per_epoch):
                ms_loss = _multistep_training_step(
                    ...
                    criterion, multistep_batch_size, current_k, multistep_seq_length, # pass current_k here
                    grad_clip, rng, weight_xy=weight_xy, weight_theta=weight_theta, weight_slip=weight_slip)
                ms_running += ms_loss
...
if __name__ == "__main__":
    ...
    if mode == '1':
        ...
        # delta_car_position_x/y are body-frame (forward/lateral) deltas now,
        # not world dx/dy -- relabel them so logs/plots aren't misleading.
        def _delta_name(c):
            if c == 'car_position_x':
                return 'delta_forward'
            if c == 'car_position_y':
                return 'delta_lateral'
            return f"delta_{c}"

        input_feature_names = CONTROL_COLS + [f"abs_{c}" for c in STATE_COLS if c not in POSITION_COLS] + [_delta_name(c) for c in STATE_COLS]
        output_feature_names = [_delta_name(c) for c in STATE_COLS]
        ...
        model = CarDynamicModel(input_size=seq_len * num_features, output_size=np.shape(outputs)[1])
        model.to(device)
        ...
        model = train_model(
            ...
            writer, device, checkpoint_dir=checkpoint_dir, patience=config.get('patience', 30),
            ...
            multistep_k_start=config.get('multistep_k_start', 5),               # Add this
            multistep_k_warmup_epochs=config.get('multistep_k_warmup_epochs', 30), # Add this
            ...
            weight_xy=config.get('weight_xy', 20.0),
            weight_theta=config.get('weight_theta', 20.0),
            weight_slip=config.get('weight_slip', 50.0),
        )
        ...
        model = CarDynamicModel(input_size=SEQ_LENGTH * (N_CONTROL + N_ABS_STATE + N_STATE),output_size=N_STATE)
